...
function AllShipmentsEdit() {
  ...
  const [allTrans, setAllTrans] = useState([]); // holds original full list
  ...
  const applySearch = (mysearchText) => {
    ...
    const filteredTrans = allTrans.filter((t) =>
      Object.values(t).some(
        (value) =>
          typeof value === "string" &&
          value.toLowerCase().includes(mysearchText.toLowerCase())
      )
    );
  ...
  const getCompletedPayments = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/getAllShipmentItems`,
        {
          params: {
                ...
                {trans.map((l, k) => {
                  return (
                    <tr key={k}>
                      ...
                      <td className="truncate">{l?.location}</td>
                      <td className="truncate">
                        {" "}
                        {Object.entries(
              ...
              <div className="p-6">
                ...
                <div className="mt-6 flex justify-end">
                  {[
                    ...
                    "PROCESSED",
                    "OUT OF OFFICE",
                  ].includes(displayingShipperInfo?.status?.toUpperCase()) && (
                    <button
...
export default AllShipmentsEdit;
