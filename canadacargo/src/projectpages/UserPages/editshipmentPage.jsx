...
export const EditShipmentPage = () => {
  ...
  const [selectedProductTypeIndex, setselectedProductTypeIndex] = useState("");
  ...
  useEffect(() => {
    setselectedProductType(selectedProductTypeIndex);
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Successful!",
        text: `Shipment has been updated.`,
      });
  ...
  const getShipmentInfoByTransId = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/getShipmentInfoForEdit`,
        {
          trans_id: params?.id,
        },
        {
      ...
      if (allData) {
        const selectedIndex = productTypes.findIndex(
          (eachtype) => eachtype?.name === allData?.product_type
        );

        setselectedProductTypeIndex(selectedIndex);
        ...
        setSelectedProvice(allData.province || "");
        setAddingDelivery(allData?.pickup_location ? "YES" : "NO");
        setSelectedLocation(allData?.pickup_location);
      } else {
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-3">
        <h4 className="text-2xl font-semibold flex pb-3 items-center text-blue-900 mt-5 px-5">
      ...
      {userToken && (
        <form
          ...
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 px-2 pt-4 pb-2 bg-gray-100 rounded-lg shadow-md px-7"
        >
          {/* Shipper Information */}
