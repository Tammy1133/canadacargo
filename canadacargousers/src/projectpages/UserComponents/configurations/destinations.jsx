...
export const Destinations = () => {
  ...
  const addNewTaxUpdate = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/createDestination`,
        {
          name: courierName,
        },
        {
  ...
  const deleteCourier = async (courierName) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/deleteDestination`,
        {
          name: courierName,
        },
        {
  ...
  const getAllTaxUpdates = async () => {
    try {
      ...
      setAllTaxRates(response.data.destinations);
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
      console.error(
  ...
  return (
    <div className="grid md:grid-cols-6 gap-4 items-start">
      <div className="p-6 min-w-[300px] md:min-w-[10px] md:col-span-2   bg-white shadow-md rounded-lg ">
        <h2 className="text-xl font-semibold mb-4">New Destination</h2>
