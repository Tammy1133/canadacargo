...
import { ClearingFeeTaxTable } from "./tables/clearingFeeTaxTable";

export const ClearingFeeTax = () => {
  ...
  const addNewTaxUpdate = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/updateClearingTaxConfiguration`,
        {
          newrate: courierName,
        },
        {
      ...
      Swal.fire({
        ...
        text: "Clearing Tax has been added to the system.",
      });
      ...
      Swal.fire({
        icon: "error",
        title: "Error Adding Clearing Tax Cost",
  ...
  const getAllTaxUpdates = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/getClearingTaxConfiguration`,
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
            "Content-Type": "application/json",
      ...
      setAllTaxRates(response.data.clearingTaxConfigurations);
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
      console.error(
  ...
  return (
    <div className="grid md:grid-cols-6 gap-4 items-start">
      ...
      <div className=" col-span-4">
        <ClearingFeeTaxTable alltaxrates={alltaxrates}></ClearingFeeTaxTable>
      </div>
      {isLoading && (
