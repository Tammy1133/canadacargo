...
import { ClearingFeeTable } from "./tables/clearingFeeTable";

export const ClearingFee = () => {
  ...
  const addNewTaxUpdate = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/updateClearingFeeConfiguration`,
        {
          newrate: courierName,
        },
        {
      ...
      Swal.fire({
        ...
        text: "Clearing Fee has been added to the system.",
      });
      ...
      Swal.fire({
        icon: "error",
        title: "Error Adding Clearing Fee",
        text: error.response?.data?.message || error.message,
      });

      console.error(
        "Error adding Clearing Fee:",
  ...
  const getAllTaxUpdates = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/getClearingFeeConfiguration`,
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
            "Content-Type": "application/json",
      ...
      setAllTaxRates(response.data.clearingFeeConfigurations);
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
      console.error(
  ...
  return (
    <div className="grid md:grid-cols-6 gap-4 items-start">
      <div className="p-6 min-w-[300px] md:min-w-[10px] md:col-span-2   bg-white shadow-md rounded-lg ">
        <h2 className="text-xl font-semibold mb-4">Update Clearing Fee</h2>
      ...
      <div className=" col-span-4">
        <ClearingFeeTable alltaxrates={alltaxrates}></ClearingFeeTable>
      </div>
      {isLoading && (
