...
export const PaymentModes = () => {
  ...
  const addNewTaxUpdate = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/createPaymentType`,
        {
      ...
      Swal.fire({
        ...
        text: "The new Payment type has been added to the system.",
      });
  ...
  const deleteCourier = async (courierName) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/deletePaymentType`,
        {
          name: courierName,
        },
        {
      ...
      Swal.fire({
        ...
        text: "The selected Payment Type has been removed from the system.",
      });
  ...
  const getAllTaxUpdates = async () => {
    try {
      ...
      setAllTaxRates(response.data.paymentModes);
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
      console.error(
  ...
  return (
    <div className="grid md:grid-cols-6 gap-4 items-start">
      <div className="p-6 min-w-[300px] md:min-w-[10px] md:col-span-2   bg-white shadow-md rounded-lg ">
        <h2 className="text-xl font-semibold mb-4">New Payment Mode</h2>
